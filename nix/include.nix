...
pkgs.stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    if [ -f "${lib}/lib/chat_plugin.dylib" ]; then
      PLUGIN_FILE="${lib}/lib/chat_plugin.dylib"
    elif [ -f "${lib}/lib/chat_plugin.so" ]; then
      PLUGIN_FILE="${lib}/lib/chat_plugin.so"
    else
      echo "Error: No chat_plugin library file found"
      exit 1
    fi
